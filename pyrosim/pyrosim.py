...
# -----Bodies----------------------------------
    ...
    def send_sphere(self,
        ...
        self._send('Sphere',
                   ...
                   x, y, z,
                   r1, r2, r3,
>>>>>>> a3611ae36e4f4ac24e7a24643143ee3425ace669
                   radius,
                   capped,
                   density,
                   *color)
...
>>>>>>> a3611ae36e4f4ac24e7a24643143ee3425ace669
        ...
        assert(len(position) == 3), "Position needs to be length 3 for x, y, z coordinates"
        try:
            len(dimensions)
        except:
            dimensions = (dimensions, dimensions)

        assert(len(dimensions) == 2), "Dimension is a Tuple"

        N, M = height_matrix.shape
        assert(N * M < 2500), "Height Matrix Can only contain 50x50 total samples"

        # compress matrix into one vector
        height_vector = height_matrix.flatten('F')
        self._send_entitiy('HeightMap',
                           *position,     # xyz position of 
                           N, M,          # matrix dimensions
                           *height_vector, # height vector
                           *dimensions,    # real width/length size
                           height_scale,  # z multiplier
                           0.0,           # offset (position makes this unnecessary)
                           1.0,           # aabb thickness, useful for thin height maps
                           0,             # infinite tiling not implemented (yet)
                           )
    ...
    def start(self):
        ...
        if self._play_paused and self._play_blind is False:
